...
- Blazor reconnect is customized in `App.razor` with `components-reconnect-modal`, `wwwroot/js/blazorReconnect.js`, and reconnect CSS in `wwwroot/app.css`; the UI should stay a minimalist bottom-center toast with a spinner plus one status line, no action buttons, and a blurred background that transitions in/out. Rejected/expired circuits auto-reload after a short delay. Blazor autostart is disabled so the script can set 15s server timeout, 5s keepalive, fast early retry intervals, and background retries after built-in retry failure.
