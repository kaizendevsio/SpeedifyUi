...
- Dashboard transport display uses `GetTransportSettingsAsync()` for the configured mode and live connected non-proxy `ConnectionItem.Protocol` stats for the actual protocol when configured mode is `auto`; avoid using aggregate `speedify` or `%proxy`/`%share` rows for active transport detection.
