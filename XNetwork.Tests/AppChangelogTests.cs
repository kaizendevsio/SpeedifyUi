...
public class AppChangelogTests
{
    ...
    {
        Assert.Equal("xbond-2026.06.96", AppChangelog.CurrentVersion);
    ...
    public void Changelog_StartsWithCurrentVersion()
    {
        ...
        Assert.Contains(entry.Changes, change => change.Contains("server egress health", StringComparison.OrdinalIgnoreCase));
        Assert.Contains(entry.Changes, change => change.Contains("Server OK", StringComparison.OrdinalIgnoreCase));
        Assert.Contains(entry.Changes, change => change.Contains("Analytics", StringComparison.OrdinalIgnoreCase));
    }
