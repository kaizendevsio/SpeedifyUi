...
public class AppChangelogTests
{
    ...
    {
        Assert.Equal("xbond-2026.06.43", AppChangelog.CurrentVersion);
    ...
    public void Changelog_StartsWithCurrentVersion()
    {
        ...
        Assert.Contains(entry.Changes, change => change.Contains("network monitor", StringComparison.OrdinalIgnoreCase));
        Assert.Contains(entry.Changes, change => change.Contains("sysfs", StringComparison.OrdinalIgnoreCase));
    }
